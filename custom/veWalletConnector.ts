...
export class VeWalletConnector extends Connector {
  ...
  private sessionListener = (_: Error | null, payload: { params: { accounts: string[]; chainId: number }[] }): void => {
    try {
      const {
  ...
  private async isomorphicInitialize(chainId = Number(Object.keys(this.rpc)[0])): Promise<void> {
    console.log(this.eagerConnection, 'check eager connection')
    ...
    await (this.eagerConnection = import('@walletconnect/client').then(async (m) => {
      ...
      this.provider.on('session_update', async (error, payload) => {
        console.log(`connector.on("session_update")`)
        if (error) {
          throw error
        }
        const { chainId, accounts } = payload.params[0]
        console.log(`connector.on("session_update")`, { chainId, accounts })
      })
  ...
  public async activate(desiredChainId?: number): Promise<void> {
    ...
    if (this.provider?.connected) {
      console.log(desiredChainId, this.provider.chainId, 'check step 2')
